<?php
  ...
  //Checks whether the password and the confirmation matches
  function is_match($password, $password_confirmation){
  ...
  //Check whether the password has at least one lower letter, one upper letter, and one special symbol
  function has_valid_password_format($value){

    return (preg_match('/[A-Z]+/', $value) && preg_match('/[a-z]+/', $value) && preg_match('/[\!$%^&]+/', $value));
  }
