<?php
  ...
  function validate_user($user, $errors=array()) {
    ...
    if(is_blank($user['password']) || is_blank($user['password_confirmation'])){
      $errors[] = "Password and password confirmation cannot be blank.";
    } elseif(!is_match($user['password'],$user['password_confirmation'])){
      ...
      $errors[] = "Password needs to have at least one lowercase letter, one upper case letter, and one special symbol[!$%^&].";
    }
  ...
  function insert_user($user) {
    ...
    $sql .= "(first_name, last_name, email, username, password_hash,created_at) ";
    $sql .= "VALUES (";
    $sql .= "'" . db_escape($db, $user['first_name']) . "',";
    $sql .= "'" . db_escape($db, $user['last_name']) . "',";
    $sql .= "'" . db_escape($db, $user['email']) . "',";
    $sql .= "'" . db_escape($db, $user['username']) . "',";
    $sql .= "'" . db_escape($db, $user['password_hash']) . "',";
    $sql .= "'" . $created_at . "'";
    $sql .= ");";
    // For INSERT statements, $result is just true/false
    $result = db_query($db, $sql);
  ...
  function update_user($user) {
    ...
    $sql .= "password_hash='" . db_escape($db, $user['password_hash']) . "' ";
